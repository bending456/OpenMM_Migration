...
def calcForce(positions,
    ...
    for i in enumerate(positions):
        if n > numberOfdead:
            dummy_coord[0][m] = i[1][0].value_in_unit(nanometers)
            dummy_coord[1][m] = i[1][1].value_in_unit(nanometers)
            dummy_coord[2][m] = i[1][2].value_in_unit(nanometers)*0
            m += 1
        n += 1
    ...
    DistCelltoOrigin          = conc.DistCelltoOrigin(recordedPositions,
                                                      ...
                                                      state)
    start_2 = timer.time()
    print('DistCelltoOrigin')
    print('|------ %s secs ------|' % (start_2-start_1))
    
    
    DistCelltoCell            = conc.DistCelltoCell(recordedPositions)
    start_3 = timer.time()
    print('DistCelltoCell')
    print('|------ %s secs ------|' % (start_3-start_2))
...
def calcStateVariable(numberOfCells,
    ...
    stateVarNew = stateVarOld + (0.0005*(1-stateVarOld) - 0.005*ConcbyCell*stateVarOld)*0.002
...
def calcForceModified(numberOfCells,
    ...
    for i in range(numberOfCells):
        ...
        if i <= numberOfdead:
            ...
            force_on_particle = [fvX[n]*mig_factor[n]*stateDepFactor[n], 
                                 fvY[n]*mig_factor[n]*stateDepFactor[n], 
