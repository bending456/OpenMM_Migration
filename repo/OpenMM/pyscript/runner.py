...
def simulator(oriConc = 10,                     # the max concentration released from the source of chemoattractant
    ...
    ATP_time_dept = open('ATP_time_dept.txt','w')
    ATP_time_dept.write('step, C_by_Cell, C_by_Origin, C_total \n')
    ...
    for num_iter in range(1, Repeat): ### What is this? 
        ...
        fvX, fvY, fvZ, ConcbyCell, odes, ConcbyOrigin, stateVariable_new, xcoord, ycoord = calc.calcForce(positions,
                                                                                                          live_num_cells,
                                                                                                          DeadCellRef1,
        ...
        if num_iter%1000 == 0:
            ATP_time_dept.write(str(num_iter)+', '+str(np.average(ConcbyCell))+', '+str(np.average(ConcbyOrigin))+', '+str(np.average(ConcbyCell+ConcbyOrigin))+'\n')
        
        
    ATP_time_dept.close()
    
    outF = open('../outputDetail.txt','a')
    outF.write(str(np.average(ConcbyCell))+', '+str(np.average(ConcbyOrigin))+', '+str(np.average(ConcbyCell+ConcbyOrigin))+'\n')
    outF.close()
