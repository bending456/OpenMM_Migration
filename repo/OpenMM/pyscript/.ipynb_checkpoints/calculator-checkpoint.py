...
def calcForce(positions,
    ...
    fvX, fvY, fvZ, odesnew, stateVariable    = conc.forceGen(ConcbyCell,
                                                             Origin,
                                                             shape_factor,
                                                             recordedPositions,
                                                             t,
    ...
    return fvX, fvY, fvZ, ConcbyCell, odesnew, ConcByOrigin, stateVariable, xcoord, ycoord
...
def calcForceModified(numberOfCells,
    ...
    ##########################################
    xcoord[xcoord <= highBC[0]-10] = 1
    xcoord[xcoord >  highBC[0]-10] = -1
    ycoord[ycoord <= highBC[1]-10] = 1
    ycoord[ycoord >  highBC[1]-10] = -1
    ...
    #for i in enumerate(positions):
    #    if n > numberOfdead:
    #        x = i[1][0].value_in_unit(nanometers)
    #        y = i[1][1].value_in_unit(nanometers)
    #        
    #        if x <= highBC[0]-10:
    #            xcoord.append(1)
    #        elif x > highBC[0]-10:
    #            xcoord.append(-1)
    #            
    #        if y <= highBC[1]-10 and y >= 0:
    #            ycoord.append(1)
    #        elif y > highBC[1]-10 or y < 0:
    #            ycoord.append(-1)
    #            
    #    n += 1
    ...
    force_x = fvX*mig_factor*xcoord
    force_y = fvY*mig_factor*ycoord
    force_z = np.zeros(len(mig_factor))
    force_live = np.transpose(np.vstack([force_x,force_y,force_z]))
    
    forces = np.vstack([force_dead,force_live])
