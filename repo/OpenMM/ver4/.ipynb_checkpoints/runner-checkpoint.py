...
def simulator(oriConc = 10,                     # the max concentration released from the source of chemoattractant
    ...
    num_dead_cell_core1 = round(numOfCells1/cellPerdead)
    num_dead_cell_core2 = round(numOfCells2/cellPerdead)
    
    dead_cell1 = []
    dead_cell2 = []
    
    for n1 in np.arange(num_dead_cell_core1):
        dead_cell1.append(randint(5,100))
        
    for n2 in np.arange(num_dead_cell_core2):
        dead_cell2.append(randint(5,100))
    
    num_dead_cell1 = np.sum(dead_cell1)
    num_dead_cell2 = np.sum(dead_cell2)
    ...
    calc.PDBgenNoPBC(pdbFileName,
                     ...
                     num_dead_cell2,
                     num_dead_cell_core1,
                     num_dead_cell_core2) # Dead cell must be placed in here first
    ...
    #RepulsiveScaleRest = 0.05
    #RepulsiveScaleAct = 0.01
    RepulsiveScaleDead = 0.1
    ...
    DeadCellRef1 = Cell_Constitution['Dead1']
    DeadCellRef2 = Cell_Constitution['Dead1'] + Cell_Constitution['Rest1'] + Cell_Constitution['Act1']
    DeadCellRef3 = Cell_Constitution['Dead1'] + Cell_Constitution['Rest1'] + Cell_Constitution['Act1'] + Cell_Constitution['Dead2']
    
    for i in range(num_particles):
        if i <= DeadCellRef1 or (i > DeadCellRef2 and i <= DeadCellRef3):
...
if __name__ == "__main__":
  ...
  for i,arg in enumerate(sys.argv):
    ...
    if arg=="-cellPerdead":
        cellPerdead = np.int(sys.argv[i+1])
