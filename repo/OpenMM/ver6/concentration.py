...
def forceGen(ConcByCell,
    ...
    if state == 'error' or state == 'linear':
        ...
        for nthCell in np.arange(NoOfCell):
            ...
            if dx <= 1e-14:
                ...
                dirFacX1[nthCell] = np.random.normal(dx,abs(dx)/10) 
                
            deltaY[nthCell] = dy
            if dy <= 1e-14:
                dirFacY1[nthCell] = 1
            else:
                dirFacY1[nthCell] = np.random.normal(dy,abs(dy)/10)
    ...
    FVectorX = DisplacementScaleByConc*(DMnewx*dirFacX1/abs(dirFacX1) + UMnewx*dirFacX2*1)
    FVectorY = DisplacementScaleByConc*(DMnewy*dirFacY1/abs(dirFacY1) + UMnewy*dirFacY2*1)
