...
def forceGen(ConcByCell,
    ...
    if dx <= 1e-14:
        ...
        dirFacX1 = np.random.normal(dx,abs(dx)/10) #/abs(probx1)
    
    if dy <= 1e-14:
        dirFacY1 = 1
    else:
        dirFacY1 = np.random.normal(dy,abs(dy)/10) #/abs(proby1)
    ...
    FVectorX = DisplacementScaleByConc*(DMnewx*dirFacX1/abs(dirFacX1) + UMnewx*dirFacX2*0.02)
    FVectorY = DisplacementScaleByConc*(DMnewy*dirFacY1/abs(dirFacY1) + UMnewy*dirFacY2*0.02)
