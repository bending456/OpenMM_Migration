...
def forceGen(ConcByCell,
    ...
    if state == 'error' or state == 'linear':
        ...
        if cellConc == 0.0:
            ...
            #    ListOfMarker = x_marker[str(nthCell)]
            #    for mthCell in np.arange(NoOfCell):
            #        if ListOfMarker[mthCell] == 0:
            #            distxmy[nthCell][mthCell] = 1e-14
            #            distxpy[nthCell][mthCell] = 1e-14
            #            distxym[nthCell][mthCell] = 1e-14
            #            distxyp[nthCell][mthCell] = 1e-14
    ...
    FVectorX = DisplacementScaleByConc*(DMnewx*dirFacX1 + UMnewx*dirFacX2*0.25)
    FVectorY = DisplacementScaleByConc*(DMnewy*dirFacY1 + UMnewy*dirFacY2*0.25)
