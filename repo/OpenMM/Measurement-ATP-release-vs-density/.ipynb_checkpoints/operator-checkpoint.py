...
if Structure == 'Slab':
    ...
    unitlength = 50
    density_list1 = [0.004]
    density_list2 = [300]#, 0.004, 0.016, 0.02]
    num_cells = [300]
    cellPerdead = 0
    DiffState = 'linear'  # error (for error function), steady, or linear
    Placement = 'near' # 'wide' 'near' 'gradient'
...
############################################
    ...
    shape_sq = 'square'
    NoParticleZone_sq = 10 # if Slab, then None
    R_of_inner_layer_sq = 50 # This can be used to assign the radial section.
    R_of_outer_layer_sq = 100
    density_list1 = [0.0001,0.0005,0.001,0.005,0.01,0.05] # maximum 0.001 with current dimension in square/round geometry
    density_list2 = [0.0001,0.0005,0.001,0.005,0.01,0.05]#,50,100]
    cellPerdead = 0
    #DiffState = 'error'
...
DiffRate = 0.01
kd = oriConc*0.8
## Autocrine and State Variable Switch 
cellconc = 0.05 # 0.0 means off!
stateVar = 'off' # or 'off'

## A Number of Simulation Related
repeat = 1
restingratio1 = 1
restingratio2 = [1]#,0.5,0.9]
    ...
    for i in np.arange(repeat):
        ...
        for ratio2 in restingratio2:
            for nocell1 in density_list1:
                for nocell2 in num_cells:#density_list2:
                    
                    cell_count1 = round(nocell1*Area)
                    cell_count2 = nocell2 #round(nocell2*Area)
