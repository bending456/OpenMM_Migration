...
def simulator(oriConc = 10,                     # the max concentration released from the source of chemoattractant
    ...
    if cellPerdead > 0:
        ...
        dead_cell = np.random.randint(10,200, size=(num_dead_cell_core))
    ...
    for num_iter in range(1, Repeat): ### What is this? 
        ...
        if num_iter == 1 or num_iter == Repeat-1:
            print('stateFactor',stateDepFactor[0])
            print('stateVar',stateVariable[0])
            print('local',ConcbyCell[0])
            print('force',fvX[0])
        
        if num_iter%5000 == 0:
            print(num_iter,'th step done')
            print('stateFactor',stateDepFactor[0])
            print('stateVar',stateVariable[0])
            print('local',ConcbyCell[0])
            print('force',fvX[0])
            
        #dummy_force_x.append(np.average(fvX[0]))#-4:]))
