...
#                                      #
########################################
Steps1 = 4                                  # number of steps within $duration1 sec simulation with stimulation on
Steps2 = 4                                 # number of steps within $duration2 sec simulation with no stimulation
Duration1 = 0.5                           # simulation length in second for stimulation-on calculation
Duration2 = 0.5                           # simulation length in second for stimulation-off calculation
boxLength = 500                             # box length in um
NoCell = 20                                 # number of cells in the box 
total_jobLength = 2                         # total job length by steps 
jobcontinued = 0                            # job you desired continued from _ This is very latest test.yml number -1
jobLength = total_jobLength - jobcontinued  # actual number of calculation steps 
autocrine = 1                               # autocrine on with other than 0 or off with 0
testmode = 0
restingCellRatio = 1
########################################
...
if jobcontinued == 0:
    if testmode == 0:
        os.system('python3 '+path+'coordGen-2D.py -outputName test0 -NoCell '+ str(NoCell)+' -restingRatio '+str(restingCellRatio))
    else:
